...
async function detectIP() {
  try {
    ...
    if (ipUsers[currentIP].blocked) {
      appendMessage('🚫 You are blocked by admin. Contact in Whatsapp 01963178893 for more information.', 'bot-message');
...
function showPremiumPopup() {
  ...
  popup.innerHTML = `<div style="background:#222;color:#fff;padding:30px;border-radius:12px;text-align:center;max-width:90%;">
    ...
    <button id="close-premium" style="padding:10px 20px;background:#555;color:#fff;border:none;border-radius:6px;">Close</button></div>`;
...
async function checkLimit() {
  ...
  if (used >= dailyLimit && !isPremiumUser) {
    appendMessage(`❌ Daily limit (${dailyLimit}) reached. Try again tomorrow or contact with admin in Whatsapp 01963178893 (message only) for premium membership`, 'bot-message');
    return false;
  }
