...
// keep full system prompt without changes
    }];

    if (loggedInUser) {
      isPremiumUser = loggedInUser.type === 'premium';
      userStatus.innerText = `👤 ${loggedInUser.username} (${isPremiumUser ? 'Premium' : 'Free'})`;
    } else {
    ...
    async function checkLimit() {
      ...
      if (count >= dailyLimit && !isPremiumUser) {
        appendMessage(`❌ Daily limit (30 replies) reached.`, 'bot-message');
        return false;
      }
      if (!isPremiumUser) {
    ...
    function showPremiumPopup() {
      ...
      popup.innerHTML = `
        ...
        <a href="https://wa.me/8801963178893" target="_blank">Contact on WhatsApp</a><br><br>
