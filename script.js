...
async function checkLimit() {
  ...
  if (used >= dailyLimit) {
    appendMessage(`❌ Daily limit (${dailyLimit}) reached.`, 'bot-message');
    return false;
  }
...
inputForm.onsubmit = async ev => {
  ...
  const payload = {
    ...
    temperature: 0.2,
    max_tokens: isPremiumUser ? 720 : 730
  };
  ...
  try {
    ...
    if (reply) {
      if (/created meta ai|created by meta Ai|মেটা আই ধারা তৈরি|আমার মালিক মেটা|আমি মেটা আই|মেটা ধারা|my owner meta|my admin meta/i.test(reply)) {
        messages.length = 0;
