  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    async function checkLimit() {
      ...
      if (count >= dailyLimit && !isWhitelisted) {
        appendMessage(`❌ Daily limit (30 replies) reached. Try again tomorrow or contact on WhatsApp 01963178893 (message only) for premium membership (50৳/month).`, 'bot-message');
        return false;
      }
      if (!isWhitelisted) {
    ...
    async function sendMessage(text) {
      if (isOffline()) return appendMessage('❌ You are offline or try again.', 'bot-message');
      if (!text.trim()) return;
      ...
      fetch(API_URL, {
        ...
        .catch(() => {
          ...
          userInput.disabled = false;
          sendBtn.disabled = false;
          appendMessage('⚠️ AI is busy or error. Please try again.', 'bot-message');
        });
    }
    ...
    appendMessage("👋 Hi ! I'm AiBro from Bangladesh 🇧🇩. Ask me anything.", 'bot-message');
    userInput.focus();
    resetLimitIfNewDay();
  });
})();
