(() => {
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- Abuse Filter (You can add your own list) ---
    ...
    // --- Premium Check with Whitelist ---
    let isPremiumUser=false,replyCount=0;
    const RATE_LIMIT_MS=7500,limitKey='reply_limit',dateKey='limit_date',dailyLimit=30;
    let lastSentTime=0,userIP='user_ip_local';
    ...
    // --- Block check (now after appendMessage) ---
    ...
    inputForm.onsubmit=async ev=>{
      ...
      const payload={model:TEXT_MODEL,messages:messages.slice(-10),temperature:0.2,max_tokens:isPremiumUser?700:730};
      try{
        ...
        if(reply){
          ...
          appendMessage('⚠️ AI is busy. Try again or contact on WhatsApp 01963178893.','bot-message');
        }
      }catch(err){
        clearInterval(iv); div.remove();
        userInput.disabled=sendBtn.disabled=false;
        appendMessage('⚠️ AI Error. Please check your internet or contact admin on WhatsApp 01963178893.','bot-message');
        console.error(err);
      }
      lastSentTime=now;
    };
