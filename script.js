(() => { document.addEventListener('DOMContentLoaded', async () => 
  ...
  if (localStorage.getItem('server_status') === 'off') {
    document.body.innerHTML = <div style="text-align:center; padding:40px;"> <h1>🔒 Server is currently closed</h1> <p>Please try again later. Contact on WhatsApp 01963178893 for more information.</p> </div>; return; 
      }
...
const abusiveWords = ['sex','porn','sexy','sexual','nude','fuck','bitch','চুদ','মাগী','খানকি'];
const containsAbuse = txt => abusiveWords.some(w => txt.toLowerCase().includes(w));
...
async function checkLimit() {
  ...
  if (used >= dailyLimit && !isPremiumUser) {
    appendMessage(`❌ Daily reply limit (${dailyLimit}) reached. Contact Whatsapp 01963178893 for premium.`, 'bot-message');
    return false;
  }
...
inputForm.onsubmit = async e => {
  ...
  if (containsAbuse(txt)) {
    appendMessage('❌ Abuse detected.', 'bot-message');
    return;
  }
  ...
  try {
    ...
    div.remove();
    userInput.disabled = sendBtn.disabled = false;
    appendMessage('⚠️ Ai error. Please try again or contact Whatsapp 01963178893', 'bot-message');
    console.error(err);
  }
  lastSentTime = now;
};
