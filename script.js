...
inputForm.onsubmit = async ev => {
  ...
  const previousMessages = userMessages.slice(-3);
  ...
  try {
    ...
    if (reply && typeof reply === 'string') {
      if (/fuck|madarchod|sex|porn|xnxx|blowjob|chud|chod/i.test(reply)) {
        messages.length = 1;
