...
if (ipData[userIP].blocked) {
  appendMessage('🚫 You are blocked. Contact admin in Whatsapp 01963178893 for more information.', 'bot-message');
...
function showPremiumPopup() {
  ...
  popup.innerHTML = `<div style="background:#222;color:#fff;padding:30px;border-radius:12px;text-align:center;max-width:90%;">
    <h2>🚀 Go Premium</h2>
    <p>Unlimited reply, faster speed for ৫০৳/month</p>
...
async function checkLimit() {
  ...
  if (used >= dailyLimit && !isPremiumUser) {
    appendMessage(`❌ Daily limit (${dailyLimit}) reached. Try again tomorrow or buy premium.`, 'bot-message');
    return false;
  }
...
inputForm.onsubmit = async ev => {
  ...
  .then(d => {
    ...
    const reply = d.choices?.[0]?.message?.content || '⚠️ AI busy. Try again later or contact on WhatsApp 01963178893.';
  ...
  .catch((e) => {
    ...
    div.remove();
    userInput.disabled = sendBtn.disabled = false;
    appendMessage('⚠️ AI error. Try again or contact on WhatsApp 01963178893.', 'bot-message');
  });
...
// --- Init ---
resetLimitIfNewDay();
appendMessage("👋 I'm AI ChatBot from Bangladesh 🇧🇩. Ask me anything!", 'bot-message');
userInput.focus();
