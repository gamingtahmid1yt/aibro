...
scrollBtn.style = 'position:fixed;bottom:80px;right:10px;background:#333;color:white;border:none;padding:6px 10px;font-size:18px;border-radius:50%;display:none;z-index:999;';
...
const whitelist = JSON.parse(localStorage.getItem('premium_whitelist') || '["37.111.212.50"]');
...
const RATE_LIMIT_MS = isPremiumUser ? 1000 : 3000;
const limitKey = 'reply_limit';
const dateKey = 'limit_date';
const dailyLimit = isPremiumUser ? Infinity : 20;
let lastSentTime = 0;
...
const previousMessages = JSON.parse(localStorage.getItem('user_messages') || '[]').slice(-10);

const aiInputMessages = [
  {
    role: 'system',
    content: "You can read user's previous messages from memory."
  },
  ...previousMessages.map(msg => ({ role: 'user', content: msg })),
  ...
  const payload = {
    ...
    temperature: 0.3,
    max_tokens: isPremiumUser ? 500 : 400
  };
